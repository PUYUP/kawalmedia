...
class AbstractAttribute(models.Model):
    ...
    content_type = models.ManyToManyField(
        ContentType, blank=True, related_name='escort_attibute',
    ...
    def _validate_option(self, value, valid_values=None):
        if not isinstance(value, get_model('escort', 'AttributeOption')):
            raise ValidationError(
...
class AbstractAttributeValue(models.Model):
    ...
    content_type = models.ForeignKey(
        ...
        on_delete=models.CASCADE,
        blank=True, null=True,
        related_name='escort_model_attibutes',
        limit_choices_to=Q(app_label='escort'))
